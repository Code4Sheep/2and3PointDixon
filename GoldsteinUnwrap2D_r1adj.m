...
if(~exist('image1'))
  load 'image1.mat'                      %Load complex image
end
im_mag   = abs(image1);                  %Magnitude image
im_phase = angle(image1);                %Phase image

%% Replace with your mask (if desired)
im_mask = ones(size(image1));
mag_max = max(im_mag(:));
%indx1 = find(im_mag < 0.1*mag_max);  %Intensity = mag^2, so this = .04 threshold on the intensity
indx1 = find(im_mag < 0.009);
...
% end
figure; imagesc(im_mag.*im_mask),   colormap("default"), axis square, axis off, title('GS Initial masked magnitude'); colorbar; watermark_image('3172');
figure; imagesc(im_phase.*im_mask), colormap(gray), axis square, axis off, title('GS Initial masked phase'); colorbar; watermark_image('3172');

%% Compute the residues
residue_charge = PhaseResidues_r1(im_phase, im_mask); % Calculate phase residues (Does not use mask)
%figure; imagesc(residue_charge), colormap("default"), axis square, axis off, title('GS Phase residues (charged)'); colorbar;
...
%figure; imagesc(branch_cuts),    colormap("default"), axis square, axis off, title('GS Branch cuts'); colorbar;
...
if(1)  % Chose starting point interactively
  ...
  %figure; imagesc(im_phase_quality,[minp maxp]), colormap("default"), colorbar, axis square, axis off; title('Phase quality map');
...
%% Unwrap
if(exist('rowref','var'))
  im_unwrapped = FloodFill_r1(im_phase, im_mag, branch_cuts, im_mask, colref, rowref)*(180/pi); % Flood fill phase unwrapping
else
  im_unwrapped = FloodFill_r1(im_phase, im_mag, branch_cuts, im_mask)*(180/pi); % Flood fill phase unwrapping
end
% Display results
figure; imagesc(im_unwrapped), colormap(gray), colorbar, axis square, axis off, title('GS Unwrapped phase'); watermark_image('3172')
